...
int main()
{
  ...
  printf("Enter the coefficients of matrix to be maximized\n");
  float cof[n+1];
  for(i=0;i<n+1;i++)
    scanf("%f",&cof[i]);
  float optimal;
  float possibl_val[count_2];
  for(i=0;i<count_2;i++)
    possibl_val[i]=0;
  optimal=-100;
  for(i=0;i<count_2;i++)
    {
      ...
      for(j=0;j<n;j++)
	{
	  possibl_val[i]=possibl_val[i]+result[i][j]*cof[j];
	}
      if(optimal<possibl_val[i] && bfs[i]==1)
	optimal=possibl_val[i];
    }
  printf("The optimal value is %f\n",optimal + cof[n]);
  printf("The optimal soultions are\n");
  for(i=0;i<count_2;i++)
    {
      if(possibl_val[i]==optimal && bfs[i]==1 )
	{
	  for(j=0;j<n;j++)
	    {
	      printf("x_%d = %f,",j+1,result[i][j]);
	    }
	}
    }
